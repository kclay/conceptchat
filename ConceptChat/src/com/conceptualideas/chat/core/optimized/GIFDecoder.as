...
package com.conceptualideas.chat.core.optimized
{
	...
	public class GIFDecoder
	{
		...
		private function getPixels(bitmap:BitmapData):Array
		{
			...
			var pixels:Vector.<uint> = new Vector.<uint>(4 * image.width * image.height);
			var count:int = 0;
			var lngWidth:int = image.width;
			var lngHeight:int = image.height;
			var color:int;

			var p:Vector.<uint> = bitmap.getVector(new Rectangle(0, 0, image.width, image.height));
			trace(pixels.length + "::" + p.length);
			/*for (var th:int = 0; th < lngHeight; th++)
			   {
			   for (var tw:int = 0; tw < lngWidth; tw++)
			 {*/
			var length:int = image.width * image.height
			...
			trace(count);
			trace(p.slice(p.length - 4));
			trace(pixels.slice(count - 4));
			return [];
		}
